        ...
        if (lhs == IARRAY) {
            if (regionflag == 0 && fraction == 1.0) {
                ...
                if(domain->nprocs >= 1 ){                    
                    ...
                    if(num_vac == 0) error->all(FLERR, "Vacancy fraction too small!!!\n");
                    while(num_vac > 0){
        ...
        if (lhs == IARRAY) {
            if (regionflag == 0 && fraction == 1.0) {
                ...
                if(domain->nprocs == 1 ){
                    ...
                    if(num_vac == 0) error->all(FLERR, "Vacancy fraction too small!!!\n");
                    vector<Point> p;          //定义一个Point类的vector 
                    vector<Point>::iterator kki;    //定义对应的迭代器 
                    Point temp_point;
                    ...
                    while(kkc<6)
                    {
                        ...
                        file>>temp_point;     //先从文件中读一个类的信息存贮到 temp_point
                        //p.push_back(temp_point);//用push_back函数存贮到vector中 
                        double kkx, kky, kkz;
                    ...
                    /*kk input test
                    if(kk_c>=1)
                        printf("kk_c %d kk_c1 %d kk_c2 %d ivalue %d\n", kk_c,kk_c1,kk_c2,ivalue);
                     */
                    if(domain->me == 0){
                        fprintf(screen, "set: vacancy count %d\n", count);
                        fprintf(logfile, "set: vacancy count %d\n", count);

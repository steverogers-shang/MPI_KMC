...
{
  //printf("kk run test!!! narg:%d stoptime:%.4f \n",narg,stoptime);
  ...
  //printf("kk run test!!! time:%.4f arg %.4f stoptime:%.4f \n",time,atof(arg[0]),stoptime);
  ...
  first_run = 0;
  if (domain->me == 0) {
    if (screen) fprintf(screen,"after setup\n");
    if (logfile) fprintf(logfile,"after setup\n");
  }
  if (stoptime > time) iterate();//deleted by bdwu 170227
  //if (1) iterate();//deleted by bdwu 170227
